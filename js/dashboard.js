...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Yükleme ekranını (video) 6 saniye sonra gizle
    const loadingScreen = document.getElementById('loading-screen');
    const loadingVideo = document.getElementById('loading-video');

    if (loadingScreen && loadingVideo) {
        console.log('Video yükleme ekranı bulundu. Gizleme zamanlayıcısı başlatılıyor (6sn)...');
        // Videonun oynatılmaya başlandığından emin ol (autoplay bazen yetmeyebilir)
        loadingVideo.play().catch(error => {
            console.warn('Video otomatik oynatılamadı, kullanıcı etkileşimi gerekebilir.', error);
            // Kullanıcı etkileşimi olmadan video oynatılamazsa,
            // yükleme ekranı yine de 6 saniye sonra gizlenir.
        });
        ...
        setTimeout(() => {
            ...
            // Ekran tamamen kaybolduktan sonra DOM'dan kaldırabiliriz (isteğe bağlı)
            setTimeout(() => {
                if (document.body.contains(loadingScreen)) { // Kaldırmadan önce varlığını kontrol et
                    loadingScreen.remove();
                    console.log('Video yükleme ekranı DOM'dan kaldırıldı.');
                }
            }, 500); // CSS transition süresi kadar bekle
        }, 6000); // 6 saniye bekle
    } else {
        console.error('Yükleme ekranı veya video elementi bulunamadı!');
    }
